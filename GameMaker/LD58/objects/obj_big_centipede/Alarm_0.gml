...
switch (state)
{
	case BIG_CENTIPEDE_STATES.waiting:
	{
		state = BIG_CENTIPEDE_STATES.attacking;
	...
	case BIG_CENTIPEDE_STATES.attacking:
	{
		state = BIG_CENTIPEDE_STATES.retreating;
		y_velocity = max_y_velocity;
		alarm[0] = 2 * game_get_speed(gamespeed_fps);
		break;
	}
	case BIG_CENTIPEDE_STATES.retreating:
	{
		state = BIG_CENTIPEDE_STATES.waiting;
		x = obj_stage_klunk.x;
		instance_create_depth(x, start_y + sprite_height / 2, depth - 1, obj_big_centipede_dust);
		y_velocity = 0;
